...
Transport* transport_create_http_malleable(MetsrvTransportHttp* config, LPDWORD size)
{
	...
	dprintf("[TIMOHELP] 108.0.1");
	dprintf("[TIMOHELP] 108.0.2 Custom header  %x", config->custom_headers[0]);
	dprintf("[TIMOHELP] 108.0.3");
	dprintf("[TIMOHELP] I'M STILL A PIECE OF SHIT AND I'M BOUT TO CRASH");
	if (config->custom_headers[0]) // THIS SHIT HERE IS MAYBE BE BUGGED TIMO
	{
	...
	//SAFE_FREE(modifiedUrl); //Whaterver, if it doesn't want to work, fuck cleaning up....
